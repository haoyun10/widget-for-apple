...
function updateEvent(event, reminder,created_event) {
...
//    event.isAllDay = true
 //   event.isAllDay = false
    ...
    var starting = new Date(reminder.completionDate)
starting.setHours(starting.getHours())
      ...
      else
      {
        if (created_event){
          if(event.isAllDay){
            ...
            console.log(`【${reminder.title}】false时间变更` )
            }else{
            reminder.dueDateIncludesTime=true
            console.log(`【${reminder.title}】true时间变更` )

            }
          if (reminder.dueDate.getDate()!=event.startDate.getDate()
          || reminder.dueDate.getHours()!=event.startDate.getHours()
          || reminder.dueDate.getMinutes()!=event.startDate.getMinutes()){
            var event_due = new Date(event.startDate)
            event_due.setHours(event_due.getHours())
            event_due.setMinutes(event_due.getMinutes())
            
            reminder.dueDate = event_due
            console.log(`【${reminder.title}】时间变更` )
